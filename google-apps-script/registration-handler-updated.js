...
function doPost(e) {
  try {
    ...
    // Append row with ALL form data including course preference
    const rowData = [
      ...
      String(data.coursePreference || 'N/A'), // M: Courses Preference
      String(data.learningPreference || 'N/A'), // N: Learning Preference
      String(data.testCenterState || 'N/A'), // O: Test Center State
      String(data.testCenterCity || 'N/A'), // P: Test Center City
      String(data.testCenterName || 'N/A'), // Q: Test Center Name
      String(data.testCenterAddress || 'N/A') // R: Test Center Address
    ];
...
COURSE PREFERENCE:
- Interested Course: ${data.coursePreference || 'N/A'}

LEARNING PREFERENCE:
- Learning Mode: ${data.learningPreference || 'N/A'}
      ...
      <pre style="background: #f5f5f5; padding: 10px; border-radius: 4px; overflow-x: auto;">
{
  ...
  "coursePreference": "AWS Certified Solutions Architect - Associate",
...
function getSubmissionStats() {
  ...
  data.forEach(row => {
    ...
    const coursePreference = row[12] || 'Unknown';
    const learningPreference = row[13] || 'Unknown';
    const testCenterState = row[14] || 'Unknown';
    
    stats.submissionsByEducationLevel[educationLevel] = (stats.submissionsByEducationLevel[educationLevel] || 0) + 1;
    stats.submissionsByCoursePreference[coursePreference] = (stats.submissionsByCoursePreference[coursePreference] || 0) + 1;
